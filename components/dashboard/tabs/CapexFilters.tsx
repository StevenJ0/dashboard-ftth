...
export interface CapexFilterState {
  distrik: string[];
  subDistrik: string[];
  wbs: string[];
  typeCapex: string[]; // CO / NEW
}

interface Props {
  onFilterChange: (filters: CapexFilterState) => void;
  availableOptions?: {
    distrik?: string[];
    subDistrik?: string[];
    wbs?: string[];
  ...
  return (
    <div ref={dropdownRef} className="relative inline-block">
      <button
        ...
        className="text-xs border border-slate-300 rounded px-3 py-2 bg-white text-slate-700 focus:ring-2 focus:ring-blue-500 outline-none cursor-pointer hover:border-blue-400 transition-colors flex items-center gap-2 whitespace-nowrap uppercase"
      >
        <span>{label}</span>
...
export default function CapexFilters({
  onFilterChange,
  availableOptions = {},
}: Props) {
  ...
  const handleFilterSelect = (
    filterKey: keyof CapexFilterState,
    value: string,
    checked: boolean
  ) => {
    // 1. Calculate new state based on CURRENT local filters
    // Note: 'filters' comes from component state (closure). 
    // This is safe for click handlers.
  ...
  const handleClearFilter = (filterKey: keyof CapexFilterState) => {
    // 1. Calculate new state
    const newFilter = { ...filters, [filterKey]: [] };
  ...
  const handleResetAll = () => {
    const emptyFilters: CapexFilterState = {
      distrik: [],
      subDistrik: [],
      wbs: [],
      typeCapex: [],
